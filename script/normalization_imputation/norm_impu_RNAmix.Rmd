...
DrImpute_impute = function(sce){
  tp = system.time({
  logcounts(sce) <- DrImpute(log(normcounts(sce)+1), mc.cores=4)
  })
...
SAVER_impute = function(sce){
  tp = system.time({
    try_res = try({
  logcounts(sce) = addminnonzerothenlog2(saver(normcounts(sce), ncores=4, size.factor=1, estimates.only = TRUE))
    })
  })
    if (class(try_res) == "try-error") {
      unname(tp)[1]=NA
