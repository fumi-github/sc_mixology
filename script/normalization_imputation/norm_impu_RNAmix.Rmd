...
linnorm_norm = function(sce){
  tp = system.time({
  logcounts(sce) = log2(exp(Linnorm(counts(sce))))
  })
...
linnormRaw_norm = function(sce){
  tp = system.time({
  logcounts(sce) = log2(Linnorm.Norm(counts(sce), output="Raw")+1)
  })
...
linnormXPM_norm = function(sce){
  tp = system.time({
  logcounts(sce) = log2(Linnorm.Norm(counts(sce), output="XPM")+1)
  })
...
mysanitymedian_impute = function(sce){
  tp = system.time({
    try_res = try({
      logcounts(sce) = log2(mysanity(2^logcounts(sce)-1,
                                     normalizemethod == "median")+1)
    })
  })
  if (class(try_res) == "try-error") {
    unname(tp)[1]=NA
...
impute_method <- list(
  ...
  SAVER=SAVER_impute,
  mysanitymedian=mysanitymedian_impute,
